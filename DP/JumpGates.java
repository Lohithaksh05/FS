...
public class JumpGates {
    public static void main(String[] args){
        ...
        System.out.println(minJumps(gates, 2, 0, dp));
                

    }

    public static int minJumps(int[] gates, int currlane, int idx, int[][] dp){
        ...
        if(currlane  != gates[idx + 1]){
            min = minJumps(gates, currlane, idx + 1, dp);
        }
        else{
            for(int i=1;i<=3;i++){
                if(currlane==i || i+1== gates[idx]) continue;
    
                min = Math.min(1 + minJumps(gates, i, idx, dp), min);
            }

        }

        return dp[idx][currlane] = min;
