...
class Day45_P3{
    public static void main(String[] args){
        ...
        System.out.println(maxScore(vals.length-1,vals,freq,dp));
    }
    
    private static int maxScore(int idx, int[] vals,Map<Integer,Integer> freq, int[] dp){
        if(idx <= 1) return freq.get(vals[idx])*vals[idx];
        
        if(dp[idx]!=0) return dp[idx];
        
        return dp[idx] = Math.max(freq.get(vals[idx])*vals[idx] + maxScore(idx-2,vals,freq,dp), maxScore(idx-1,vals,freq,dp));
    }
